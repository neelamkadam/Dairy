...
// TODO: backend will supply the entry mode (manual/auto) per record.
// Until then, derive a stable dummy value from the farmer code.
const getEntryMode = (record: any, index: number): "Manual" | "Auto" => {
  const code = parseInt(record?.code, 10);
  return (isNaN(code) ? index : code) % 2 === 0 ? "Auto" : "Manual";
};

const ManualAutoShiftReport: React.FC = () => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Manual_Auto_Shift_Report_${vlcName}_${dateStr}_${formData.shift}.pdf`);
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <Table className="border-none text-center">
          ...
          <TableBody>
            {paginatedData.length > 0 ? paginatedData.map((record: any, index: number) => {
              const entryMode = getEntryMode(record, startIndex + index);
              return (
              <TableRow
      ...
      <div style={{ position: "absolute", left: "-9999px", top: 0 }}>
        <div ref={pdfExportRef} className="p-4 bg-[#ffffff]" style={{ width: "1200px", fontFamily: "'Noto Sans Devanagari', 'Roboto', sans-serif", color: "#000000" }}>
          ...
          <h1 className="text-2xl font-bold text-center mb-1">Manual/Auto Shift Report</h1>
          ...
          <table className="w-full border-collapse border border-[#9ca3af]">
            <thead>
              <tr className="bg-[#f3f4f6]">
                ...
                <th className="border border-[#9ca3af] py-1 px-3 text-sm font-bold">Entry Mode</th>
              </tr>
            </thead>
            <tbody>
              {farmerData.map((record: any, index: number) => (
                <tr key={index} className={index % 2 === 0 ? "bg-[#ffffff]" : "bg-[#f9fafb]"}>
                  ...
                  <td className="border border-[#9ca3af] py-1 px-3 text-center text-sm">{getEntryMode(record, index)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default ManualAutoShiftReport;
