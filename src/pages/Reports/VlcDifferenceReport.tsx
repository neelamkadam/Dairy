...
const VlcDifferenceReport = () => {
  ...
  const processVlcRawData = (rawData: any[]) => {
        ...
        Object.values(groups).forEach((items) => {
          ...
          // Only synthesize 'Both' from Cow + Buffalo when those rows actually carry data.
          // Otherwise (e.g. VLC entered combined data only), keep the backend's 'Both' row as-is.
          const cowBuffaloHasData =
            parseFloat(cow?.vlc?.total_weight || 0) > 0 ||
            parseFloat(buffalo?.vlc?.total_weight || 0) > 0 ||
            parseFloat(cow?.dairy?.total_weight || 0) > 0 ||
            parseFloat(buffalo?.dairy?.total_weight || 0) > 0;

          // Recalculate 'Both' when the backend one is missing/zero, but only if Cow+Buffalo have real data.
          const shouldCalculateBoth = cow && buffalo && cowBuffaloHasData && (!backendBoth ||
  ...
  const handleExportExcel = () => {
    ...
    ws['!cols'] = [{ wch: 30 }, ...Array(18).fill({ wch: 12 })];
