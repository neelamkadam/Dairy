...
const BMCCollection = () => {
  ...
  const toggleSelectAll = () => {
    if (selectedEntities.length === entities.length) {
      ...
      setSelectedEntities(entities.map(e => Number(e.bmc_id ?? e.id)));
    }
  };
  ...
  const handleApplyFilters = async () => {
    ...
    if (selectedEntities.length === 0) {
      toast.error("Please select at least one BMC");
      return;
    }
    ...
    try {
      ...
      if (allVlcIds.size === 0) {
        toast.info("No dairies found under the selected BMCs.");
        setFarmerData([]);
        setCurrentPage(1);
        return;
      }
  ...
  const handleExportToExcel = () => {
    const exportData = aggregatedData.map((data) => {
      const entity = entities.find(e => Number(e.bmc_id ?? e.id) === data.entityId);
      ...
      return {
        'BMC ID': entity?.bmc_id ?? entity?.id ?? data.entityId,
        'BMC Name': entity?.name || 'N/A',
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'BMC Collection');
    XLSX.writeFile(wb, `BMC_Collection_${format(new Date(), 'dd-MM-yyyy')}.xlsx`);
              ...
              <Popover open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    {selectedEntities.length === 0 
                      ? `Select BMCs` 
                      : `${selectedEntities.length} BMC(s) selected`}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-80 p-0 bg-white" align="start">
                  ...
                  <div className="max-h-60 overflow-y-auto p-2">
                    {entities?.map((e) => {
                      const eId = Number(e.bmc_id ?? e.id);
                      return (
                        <div
                          key={eId}
      ...
      <Card className="border-0 shadow-lg bg-white">
        <CardHeader className="border-b">
          <div className="flex flex-wrap justify-between items-center gap-4">
            <CardTitle className="text-xl font-semibold text-gray-800">BMC List</CardTitle>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50">
                  <TableHead className="text-gray-700 font-semibold px-4 py-3">BMC ID</TableHead>
                  <TableHead className="text-gray-700 font-semibold px-4 py-3">BMC Name</TableHead>
                  ...
                  return (
                    <TableRow key={index} className="hover:bg-blue-100 transition-colors">
                      <TableCell className="font-medium px-4 py-3">
                        {entity?.bmc_id ?? entity?.id ?? data.entityId}
                      </TableCell>
